...
# (rather than indiegogo-specific) on purpose: a real row here is a real
# crowdfunding campaign, and there is no fixed set of "fake-looking" names to
...
def sample_check():
    ...
    # It is a Campaign-mode sample (--mode search) — see output_writer.py
    # for why Campaign and Reward are two different dataclasses here rather
    # than the single Product this check imports in this family's e-commerce
    # members.
    sys.path.insert(0, str(REPO))
    from dataclasses import asdict

    from output_writer import Campaign
    expected = list(asdict(Campaign()).keys())

    for i, row in enumerate(rows):
        if list(row.keys()) != expected:
            failed.append(f"sample_output.json row {i}: columns differ from "
                          f"output_writer.Campaign")
            break

    with (REPO / "sample_output.csv").open(newline="", encoding="utf-8") as handle:
        header = next(csv.reader(handle))
    if header != expected:
        failed.append("sample_output.csv header differs from output_writer.Campaign")
