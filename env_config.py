...
ENV_KEYS = {
    ...
    "INDIEGOGO_CDP_ENDPOINT": "cdp_endpoint",
    "INDIEGOGO_PROXY": "proxy",
    "INDIEGOGO_URL": "url",
}
...
#
# `INDIEGOGO_URL` IS here, and that only works because each engine declares
# `--url` with `default=None` and resolves its fallback AFTER calling
# apply(). Give `--url` an argparse default and this entry becomes inert in
# exactly the way the paragraph above describes, with nothing failing to say
# so. There is a check in smoke_test.py pinning that.
...
# .env.example documents INDIEGOGO_CDP_ENDPOINT and INDIEGOGO_PROXY
...
def duplicate_keys(path=None):
    ...
    2026-09-17 on a fixture with three consecutive INDIEGOGO_PROXY lines,
...
def unknown_keys(path=None):
    ...
    Usually a typo — `TWO_CAPTCHA_KEY`, `INDIEGOGO_CDP` — which otherwise fails
