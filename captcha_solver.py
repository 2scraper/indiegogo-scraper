...
scrapers, regardless of what URL was requested (a search listing, a campaign
page, a rewards tab, anything) — deliberately, not scoped to any one page.

**THIS MODULE IS LOAD-BEARING ON THIS SITE, and this is the first repo in
the family where that was true on the very first request.** Every HTML route
on indiegogo.com answered a datacentre address with an HTTP 403 Cloudflare
managed challenge on 2026-09-17 — the home page, the search listing, a
campaign page, `robots.txt` and `sitemap.xml` alike, and the site's own JSON
API with them. There is no ungated route to fall back to the way
bbb-scraper has one. The response announces itself twice: a
`cf-mitigated: challenge` header, and a body carrying
`challenges.cloudflare.com` five or six times.

That challenge is a Cloudflare **Turnstile**, and 2Captcha solves it with
`TurnstileTaskProxyless`. Writing anything weaker than that — "the solver
does not help on this site" — would be the single most expensive sentence
this repo could ship, because it is invisible to every test and tells a
reader not to buy something that works. The only sentence this file is
entitled to is which task types it IMPLEMENTS.

Why a Cloudflare Challenge page needs an interception, not a parser
-------------------------------------------------------------------
A Turnstile on a Cloudflare **Challenge** page publishes no sitekey
anywhere in its markup. Cloudflare calls `turnstile.render(container,
params)` exactly once and keeps nothing: `sitekey`, `action`, `cData` and
`chlPageData` exist only inside that call, and `TurnstileTaskProxyless`
needs all four. So a static read of the HTML — any static read, however
careful — cannot produce a solvable task.

The answer is an init script installed on the context BEFORE any page script
runs, hooking `turnstile.render` and recording its arguments
(`TURNSTILE_INTERCEPT_JS` below). That is the one piece each driver spells
its own way, so it cannot live in this file:

    Playwright   context.add_init_script(js)
    pyppeteer    page.evaluateOnNewDocument(js)
    Selenium     CDP Page.addScriptToEvaluateOnNewDocument

Three details that were each a bug first, in the sibling repo this path came
from: poll for the capture rather than reading once; RAISE rather than
building a task from a sitekey-less detection (paying for a request the API
will reject is worse than reporting the page unsolved); and the user agent
did NOT have to match the browser's, so log a mismatch to rule it out rather
than building a fingerprint pipeline for a problem nobody has measured.

It is still not the FIRST thing to reach for
---------------------------------------------
Over the Scraping Browser API, `Captcha.setAutoSolve` clears this challenge
inside the browser before the solver API is ever offered it — that is the
PRIMARY path in this repo and the reason a live run needs no key of its own.
Hence `--solve-captcha when-blocked` as the default, with the retry budget
ahead of any spending.
...
NARROW: `--solve-captcha when-blocked` is the default and counts results
before paying, and `page_flow.STATE_POLICY` — not this file — decides which
state is worth money at all. In particular a `blocked` page with no widget
...
def detect_recaptcha_in_page(evaluate, page_url: str = "") -> Optional[CaptchaChallenge]:
    ...
    Version inference, and why it is deliberately conservative:
      ...
      * an interactive bframe present with NO size and NO render parameter is
        ...
        alone rules it out. A PerimeterX-style denial page is exactly
    ...
    if render and render != "explicit" and render == info.get("sitekey"):
        ...
        #
        # This rung was absent, and its absence cost a real solve in a sibling repo:
